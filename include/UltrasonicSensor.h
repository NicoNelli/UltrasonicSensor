...
public:
	...
	bool GetValidity() const; //It checks if the result is valid(based on the expired timeout )	
...
private:
	...
	void TimeOfTravel(); 
	//It computes the amount of time it takes for the pulse to go and come back.

	int trigger; //trigger pin

	int echo; //echo pin

	volatile long startTimeUsec; //start timing

	volatile long endTimeUsec; //end timing
...
/*It allows to estimate the velocity based on two measurements (difference quotient).
Parameters:
1) z position of the platform
2) previous z position of the platform
3) FIFO queue.
4) flag to control the first time the function is called.
5) time object to obtain dt for computing the difference quotient. 
*/
...
/*
Kalman filter with one state variable which is the distance obtained by the sensor.
...
double movingAvarageFilter(double distance, std::queue<double> *buffer, double *currentValue, int window);
